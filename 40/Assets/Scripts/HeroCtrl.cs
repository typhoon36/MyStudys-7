...
public class HeroCtrl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (m_NickName != null)
            m_NickName.text = PlayerPrefs.GetString("UserNick", "사냥꾼");
    }
...
#region ---- 마우스 클릭 이동
   ...
   // float m_Tick = 0.0f;
    ...
    {  // 클릭 이벤트가 발생했을 때 이 함수를 호출합니다.
        ...
        Sound_Mgr.Inst.PlayGUISound("gun", 0.4f);
    }
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.name.Contains("BulletPrefab") == true)
        {
            ...
            if (other.gameObject.GetComponent<BulletCtrl>().m_IsPool == false)
    ...
    void TakeDamage(float a_Value)
    {
        ...
        if(m_CurHp <= 0.0f)  //사망처리
        {
            ...
            //게임오버
            Game_Mgr.Inst.OpenGameOverPanel();
        }
    }
    ...
    void UseBombSkill()
    {
        ...
        Sound_Mgr.Inst.PlayEffectSound("explosion_deep_low_1", 0.3f);
